...
uv sync --extra twitter     # Install optional Twitter Playwright scraper
...
- `scrapers/` — 10 sources (rss, hackernews, reddit, telegram, twitter, github, openbb, ossinsight, gdelt, google_news), all async via `httpx.AsyncClient`, inherit `BaseScraper`
...
- Twitter source has two scraper implementations: `twitter.py` (Apify actor `altimis~scweet`, requires `APIFY_TOKEN` env var) and `twitter_playwright.py` (cookie-based, requires `uv sync --extra twitter` for playwright + playwright-stealth).
...
**The Pi cron job is the live production path** — see [`docs/pi-daily-run.md`](docs/pi-daily-run.md) for the confirmed end-to-end flow (cron → docker compose → pipeline → pi-deploy.sh → gh-pages) with a Mermaid diagram. The GitHub Actions workflow `daily-summary.yml` exists in the repo but has never actually run (no secrets configured, zero run history as of 2026-06-30) — don't assume it's the source of daily output.
